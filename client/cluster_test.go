...
var nodeCfgPath, pdCfgPath []string
var pdServers [3]*pd.Server
var nodeServers [3]*storage.NodeServer
var pdHost string
...
func init() {
	...
	pdCfgPath = []string{"../config/pd8001.cfg"}

	runtime.GOMAXPROCS(runtime.NumCPU())

	for i, path := range pdCfgPath {
		go startPdServer(path, i)
	}

	time.Sleep(3 * time.Second)
	initMetaData()

	for i, path := range nodeCfgPath {
		go startNodeServer(path, i)
...
func startPdServer(configPath string, index int) {
	...
	pdHost = cfg.GetString(config.SectionDefault, "Host")
	pdCfg.Host = pdHost
	...
	pdServers[index], err = pd.NewServer(pdCfg, false)
	if err := pdServers[index].Start(); err != nil {
		panic(err.Error())
	}
}
...
func initMetaData() {
	log.Infof("pd host: %s\n", pdHost)
	c, err := meta.NewNodeClient(pdHost)
	if err != nil {
		panic(err.Error())
	}
	defer c.Close()

	nodeDescriptors := []meta.NodeDescriptor{
		{NodeID: 1, Address: "127.0.0.1:9001"},
		{NodeID: 2, Address: "127.0.0.1:9002"},
		{NodeID: 3, Address: "127.0.0.1:9003"}}

	bc := meta.BatchRequest{}
	bc.Add(&meta.SetNodeDescriptorsRequest{NodeDescriptor: nodeDescriptors})
	br := meta.BatchResponse{}

	resp, err := c.NodeMessage(context.TODO(), &bc)
	if err != nil {
		log.Fatal(err.Error())
	}
	br = *resp
	if br.Resp[0].SetNodeDescriptors.GetError() != nil {
		log.Fatalf("set node descriptors failed, error: %v", br.Resp[0].SetNodeDescriptors.GetError())
	}

	var rds []meta.RangeDescriptor

	ReplicaDescriptors := []meta.ReplicaDescriptor{
	...
	rd = meta.RangeDescriptor{
		RangeID:  20,
		StartKey: meta.Key("\x02"),
		EndKey:   meta.Key("\x02\xaa"),
		Replicas: ReplicaDescriptors}
	rds = append(rds, rd)

	rd = meta.RangeDescriptor{
		RangeID:  30,
		StartKey: meta.Key("\x02\xaa"),
		EndKey:   meta.Key("\x03"),
		Replicas: ReplicaDescriptors}
	rds = append(rds, rd)

	bc.Reset()
	bc.Add(&meta.SetRangeDescriptorsRequest{RangeDescriptor: rds})
	br.Reset()
	...
	br = *resp
	if br.Resp[0].SetRangeDescriptors.GetError() != nil {
		log.Fatalf("set range descriptors failed, error: %v", br.Resp[0].SetRangeDescriptors.GetError())
	}
}

func startNodeServer(configPath string, index int) {
	if !util.IsFileExists(configPath) {
...
func (s *testKVSuite) TestIsolationMultiRange(c *C) {
	...
	keys := make([][]byte, 0, keyCnt)
	keys = append(keys, []byte("\x00_test_key_1"))
	keys = append(keys, []byte("\xbb_test_key_2"))
